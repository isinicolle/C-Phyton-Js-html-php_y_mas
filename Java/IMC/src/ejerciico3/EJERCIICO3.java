
package ejerciico3;

import java.text.DecimalFormat;
import java.util.Scanner;


public class EJERCIICO3 {

   
    public static void main(String[] args) 
    {
      DecimalFormat ndec= new DecimalFormat("#.00");
      Scanner scan = new Scanner (System.in);
      
      
      String nombre;
      String Observacion;
      String ObsGen = null;
      int edad, contador=0;
      char genero;
      double peso, estat, imc;
     
     do
     { 
         contador++;
         
        System.out.println("\t\t Indice de masa corporal");
        System.out.print("Ingrese el nombre: ");
        nombre=scan.nextLine();
        
        do{
            
        System.out.print("Ingrese la edad: ");
        edad=scan.nextInt();
        
            if(edad<1 || edad>85)
                System.out.println("Ingrese una edad valida entre 1 y 85");
                        
        }while(edad<1 || edad>85);
        
        do{
            
       
        System.out.print("Ingrese su genero: ");
        genero=scan.next().charAt(0);
        genero=Character.toUpperCase(genero);
        
            if(genero!='F' && genero!='M')
                System.out.println("Ingrese una opcion valida F/M");
            
        }while(genero!='F' && genero!='M'); 
        System.out.print("Ingrese su peso: ");
        peso=scan.nextDouble();
        System.out.print("Ingrese su estatura: ");
        estat=scan.nextDouble();
        
        imc=peso/(estat* estat);
        
        if(imc<18)
        {
            Observacion = "Bajo peso";
            
        }
        else if (imc>25)
        {
            
            Observacion= "Bajo peso";
        
        }
        else 
        {
            Observacion= "Buen peso";
        }
       
        switch(genero)
        {
           case 'F':
               ObsGen="Fememnino";
           break;
           case 'M':
               ObsGen="Masculino";
            break;
           default :
 
               
               
        }
        
        System.out.println("\t\t DATOS INGRESADOS");
        System.out.println("Su nombre es: "+nombre);
        System.out.println("Su geenro es: "+genero);
        System.out.println("Tipo de genero: "+ObsGen);
        System.out.println("Su edad es: "+edad);
        System.out.println("Su peso es: "+ndec.format(peso));
        System.out.println("Su estatura es: "+ndec.format(estat));
        System.out.println("Su Indice de masa corporal es: "+ndec.format(imc));
        System.out.println("Estado: "+Observacion);
         scan.nextLine(); //Limpiar el buffer
         
          
     
     }while(contador<3);
      
         
    }
    
}
